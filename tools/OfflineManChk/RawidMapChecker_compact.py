#!/usr/bin/env python3
# RawidMapChecker.py の表示違いバージョン。
# テーブル表示では rawid1 を "rawid" とラベルし、zone と rawid2 の列を表示しない。
# （データの読み込み・出力フォーマットは元のバージョンと同じ）
#
...
# テーブルに表示する列と、表示ラベル（rawid1 -> rawid、zone / rawid2 は非表示）
DISPLAY_FIELDS = ["PL", "eventid", "rawid1", "ax", "ay", "x", "y", "ph"]
DISPLAY_LABELS = {"rawid1": "rawid"}
...
class RawidMapChecker(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Rawidmap Checker (compact)")
